"""
Analyzes a complex SQL query to extract hierarchical relationships from CASE statements
...
# This setup is necessary to run this script standalone
project_root = Path(__file__).parent.resolve()
sys.path.insert(0, str(project_root / "libs" / "databridge-core"))
sys.path.insert(0, str(project_root / "libs" / "databridge-models"))
sys.path.insert(0, str(project_root / "libs" / "databridge-discovery" / "src"))

from fastmcp import FastMCP
from databridge_discovery.mcp import register_discovery_tools

def run_sql_analysis():
    """
    Initializes the discovery tools, analyzes the SQL, and exports the hierarchy.
    """
    print("-- Starting SQL Hierarchy Analysis --")

    # 1. Initialize a temporary MCP instance and register discovery tools
    mcp = FastMCP()
    register_discovery_tools(mcp)
    print("[Step 1] Initialized discovery tools.")
    ...
    print("[Step 2] Parsing SQL...")
    parse_tool = mcp._tool_manager._tools["parse_sql"]
    parse_result = parse_tool.function(sql=sql_query, dialect="snowflake")

    if not parse_result.get("success"):
        print("SQL parsing failed:", parse_result.get("error"))
        return

    print("[Step 3] Extracting CASE statements...")
    extract_tool = mcp._tool_manager._tools["extract_case_statements"]
    extract_result = extract_tool.function(sql=sql_query, dialect="snowflake")

    if not extract_result.get("success"):
        print("CASE statement extraction failed:", extract_result.get("error"))
        return

    # 4. Process and export the results
    print("[Step 4] Processing and exporting results...")
    all_rules = []
    for case_info in extract_result.get("case_statements", []):
        column_name = case_info.get("column_name")
        for rule in case_info.get("rules", []):
            all_rules.append({
                "hierarchy_name": column_name,
                "parent_value": rule.get("result"),
                "child_value": rule.get("condition"),
            })

    if not all_rules:
        print("No hierarchy rules were extracted.")
        return

    df = pd.DataFrame(all_rules)
    output_path = "proposed_hierarchies.csv"
    df.to_csv(output_path, index=False)

    print(f"\n--- Analysis Complete ---")
    print(f"Successfully extracted {len(df)} hierarchy rules.")

...
The existing plans for Librarian and Researcher already define a powerful set of skills:
...
*   **Tools:** It uses Git tools to commit the project to a repository, and it uses dbt Core or dbt Cloud APIs to run `dbt build` and `dbt test`. It then uses Researcher's analytics tools to run post-deployment validation queries (e.g., row count comparisons).

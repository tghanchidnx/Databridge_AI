...
## V3: Headless DataBridge AI (The Hierarchy & Reconciliation Engine)
...
In essence, **V3 is used to define the "what" and "how" of the data**—what the business concepts are and how they map to the underlying data systems.

## V4: Headless DataBridge Analytics (The Analytics & Insights Engine)

Version 4 is the analytical engine that sits on top of the foundation laid by V3. It leverages the hierarchies and mappings created in V3 to perform powerful, context-aware analysis on large datasets.

### Key Responsibilities of V4:

*   **Compute Pushdown:** This is the core architectural principle of V4. Instead of pulling massive datasets out of a data warehouse to be analyzed locally, V4 generates and sends optimized SQL queries to the data warehouse (like Snowflake or Databricks) for execution. Only the much smaller, aggregated results are returned. This is highly efficient and scalable.
...
*   **Insight Generation:** V4 goes beyond just answering questions. It can proactively analyze data to:
...
In essence, **V4 is used to analyze the data and answer the "why"**—why did sales increase, what are the key drivers of variance, what are the emerging trends.

## Synergy and Overall Purpose

The two versions work in tandem to create a comprehensive, AI-driven FP&A platform:

1.  **V3** is used by data stewards, financial systems analysts, or developers to build and maintain the logical data model (the hierarchies).
2.  **V4** is used by financial analysts, business users, and executives to query and analyze data in a self-service manner, using the business terms defined in V3, often through a natural language interface.
